...
int main(){
    ...
    printf("2. View All Dishes\n");
    printf("3. Searches dishes\n");
    printf("4. Cancel Order\n");
    printf("5. view all orders\n");
    printf("6. Exit Customer Panel\n");
    printf("Enter your choice: ");
    scanf("%d", &choice);
    switch (choice)
    {
    ...
    /*save dish in database*/
     ...
     void saveDataDish(struct Dish dishes[])
    {
    FILE *dishFile = fopen("dishes.txt", "w");
    if (dishFile != NULL)
    {
...
/*view all dishes */
    ...
    {
        if(dishCount == 0){
            printf("no dishes are available.\n");
            return;
        }
    printf("\n=== List of All Dishes ===\n");
    printf("%-10s %-30s %-50s %-10s %-20s\n", "ID", "Name", "Description",
    "Price", "Preparation Time (min)");
    printf("--------------------------------------------------------------------------------------------------------------------------------------------\n");
...
void searchDishes(struct Dish dishes[])
{
    ...
    for(int i=0;i<dishCount ;i++){
        if(strcmp(strlwr(dishes[i].name),strlwr(query))==0 || strcmp(strlwr(dishes[i].id),
                        strlwr(query))==0)
        {
            printf("Dish ID: %s\n", dishes[i].id);
            printf("Name: %s\n", dishes[i].name);
...
void viewSalesReport(struct Order orders[], struct Dish dishes[])
    ...
    int dishSales[MAX_DISHES] = {0};

    char dishNames[MAX_DISHES][MAX_NAME_LENGTH];
    ...
    {
    dishSales[j]++;
    totalSales += dishes[j].price; // Add the price of the

    break; // Found the dish, no
    ...
    float dishTotal = dishSales[i] * dishes[i].price; // Calculate
    ...
    sprintf(newOrder.id, "ORD%d", orderCount + 1); // Generate a

    newOrder.status = 1; // Set status

    orders[orderCount] = newOrder; // Add the new
